...
/// Resolves a path against the *document*, and hands the worker absolute URLs.
///
/// This is load-bearing. A relative URL fetched inside a worker resolves against
/// the worker script's own location, not the page — so `assets/...` would become
/// `/python/assets/...`, miss, and the dev server would answer the 404 with
/// index.html. The wasm compiler then reports `expected magic word 00 61 73 6d,
/// found 3c 21 44 4f`, which is `<!DO` — the start of an HTML document.
///
/// Going through `baseURI` rather than assuming a leading slash also keeps this
/// correct when the app is deployed under a sub-path with `--base-href`.
...
///
/// The worker is not about keeping the UI smooth — it is the stop button.
/// A program that never returns cannot be asked to stop, so the only reliable
/// answer is to terminate the whole worker from outside, which is exactly what
/// [cancel] does. Everything else here follows from that: the worker is
/// disposable, and a fresh one is spun up on demand.
class WebPythonRuntime implements PythonRuntime {
  ...
  Future<void> cancel() async {
    ...
    // Terminate rather than ask. The program may be in a loop that never yields,
