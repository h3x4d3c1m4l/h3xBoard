      ...
      wasi_snapshot_preview1: {
        ...
        poll_oneoff: (subsPtr, eventsPtr, count, neventsPtr) => {
          ...
          if (!readyNow && earliest !== null) {
            // A spin, because there is no synchronous sleep in a worker without
            // SharedArrayBuffer, and SharedArrayBuffer needs COOP/COEP headers
            // on the whole app — which would break the cross-origin font loads.
            // It burns a worker thread that has nothing else to do, and Stop
            // still works because that terminates the worker outright.
