...
class ExternalDisplay {
  ...
  /// 取得外接顯示器支援的解像度列表 ("WxH", 像素). Defaults to the first external
  /// screen. Returns an empty list on platforms/screens without mode info.
  Future<List<String>> getModes({int? targetScreen}) async {
    final modes = await _displayController
        .invokeMethod('getModes', {"targetScreen": targetScreen});
    return (modes as List).cast<String>();
  }
  ...
  /// 連接外接顯示器並取得分辨率. When [width]/[height] are given the external
  /// screen uses that resolution if available; otherwise the highest-resolution
  /// mode is chosen.
  Future connect({String? routeName, int? targetScreen, int? width, int? height}) async {
    final size = await _displayController.invokeMethod('connect', {
      "routeName": routeName,
      "targetScreen": targetScreen,
      "width": width,
      "height": height,
