...
# The stdlib ships as one zip rather than thousands of loose files: Flutter would
# otherwise need every one listed in pubspec.yaml, and CPython reads a zip on
# sys.path natively through zipimport.
#
...
# Stored entries need no zlib. It costs disk but almost nothing over the wire,
# since the .py text gzips well in transit anyway (12 MB -> 4.1 MB).
STDLIB_ZIP_ABS="$(cd "$OUT_DIR" && pwd)/python${PYTHON_VERSION%.*}.zip"
rm -f "$STDLIB_ZIP_ABS"
echo "==> packing stdlib (stored)"
( cd "$SRC_DIR/Lib" && zip -q -r -X -0 "$STDLIB_ZIP_ABS" . \
    -x '*.pyc' 'test/*' 'idlelib/*' 'tkinter/*' 'turtledemo/*' 'lib2to3/*' '*__pycache__*' )
